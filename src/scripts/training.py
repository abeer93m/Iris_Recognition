...
from resnet50_model import ResNet50Embedding, TripletLoss, initialize_model
import wandb
from PIL import Image
import numpy as np

# Initialize wandb
wandb.init(project="iris-identification", config={
    "epochs": 25,
    "batch_size": 8,
    "learning_rate": 1e-4,
    "k_folds": 5,
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_size, num_epochs=25, save_dir='D:/Iris Identification/IRIS_IDentification/checkpoints', accumulation_steps=4):
    best_loss = float('inf')
...
if __name__ == "__main__":
    ...
    # Parameters
    data_dir = 'D:/Iris Identification/IRIS_IDentification/data/processed_normalized_100'
    triplet_file = 'D:/Iris Identification/IRIS_IDentification/data/triplets.txt'
    config = wandb.config
    ...
    model, criterion, optimizer = initialize_model(embedding_dim=config.embedding_dim, margin=config.margin)
